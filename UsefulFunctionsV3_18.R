#-------------------------------------------------------------------------------#
#-------------------------------------------------------------------------------#
#	All core R-code developed so far were cleaned, improved, and saved 
#	as functions in this file for better readability & revision tracking 				
#																				#
#    Start:		Aug 25, 2014							     					#
#    End:		Sep 11, 2014														#
#    R Version:	        3.4.1 					     	
...
#-------------------------------------------------------------------------------#
#	Dependency/Call structure ...
#		# 11->(9, 2->(1), 6, 7)
#		# 12->(8)
#		# 3->(1)
#		# 4->(1)
#		# 10 an optional improvement of #7 to display group weights info on the plot
#		# 13 ->(8)
#		# 14 ->(8,1)
#		 
#-------------------
...
#-----------------------------------------------------------------------------------------------------
# sub-setting the return data for the specified data range and handles NAs: 21-05-2018
#-----------------------------------------------------------------------------------------------------
...
#		fastMveEst <- function(x, spec = NULL, ...) mveEstimator
	...
	else if (grp==4)  { # Conservative Portfolio with individual fund wt capped at 0.8
		if (is.null(col.lgnd)) stop("must provide column legend info when grp !=0")
		grp.names = col.lgnd$Fund.Category[ match(colnames(my_asset), col.lgnd$Legends)]
		pfConstraints = c(# "LongOnly", ### warning - not working along with fund-level constraints!! 
		paste("minW[1:",dim(my_asset)[2],"]=0.0",sep=""),
	...
	else if (grp==4.1)  { # Conservative Portfolio with individual fund wt capped at 0.4
		if (is.null(col.lgnd)) stop("must provide column legend info when grp !=0")
		grp.names = col.lgnd$Fund.Category[ match(colnames(my_asset), col.lgnd$Legends)]
		pfConstraints = c(# "LongOnly", ### warning - not working along with fund-level constraints!! 
		paste("minW[1:",dim(my_asset)[2],"]=0.0",sep=""),
	...
	else if (grp==4.2)  { # Conservative Portfolio with individual fund wt capped at 0.2
		if (is.null(col.lgnd)) stop("must provide column legend info when grp !=0")
		grp.names = col.lgnd$Fund.Category[ match(colnames(my_asset), col.lgnd$Legends)]
		pfConstraints = c(# "LongOnly", ### warning - not working along with fund-level constraints!! 
		paste("minW[1:",dim(my_asset)[2],"]=0.0",sep=""),
...
#------------ 22-05-2018 -----------------------------------------------------------------------------
 	pfBacktest_V2 = function(a.pf, RET=NULL, start=NULL, end=NULL, dbg=0)
	{
		...
		AnnRet = t(table.AnnualizedReturns(dta1, scale=261))[1]
		AnnRsk = t(table.AnnualizedReturns(dta1, scale=261))[2]
...
#--------------------------------------------------------------------------------------------------------#	
	...
	#------	print(length(wts)) ----------------------------------------------------------------------------#
		...
		wtsMatPos = round(wtsMat[, colPos],4)
...
#----------------------------------------------------------------------------------------------------------#
	myplot5.Wts = function(weightmat, collst =  topo.colors(6)[c(1,2,3)], ftyp = lgnd)
	{
		nr = dim(weightmat)[1]
...
#----------------------------------------------------------------------------------------------------------#
	myplot6.Wts = function(weightmat, collst =  topo.colors(6)[c(1,2,3)], ftyp = lgnd)
	{
		nr = dim(weightmat)[1]
...
# spent some time on this function on Sept 1 but final outcome is poor - so going back to v5 of myplot. 
	...
	#-------------------------------------------------------------------------------#
	SMart_Wrapper_ConservativeOptPF = function(hor = 12, group = 4, method = "Basic", token = "optCPF3h1yr")
	{
		tme = BackTestingTimeFrames(horizon = hor) # computes the time windows for construction & testing 
		if (!(group %in% c(4, 4.1, 4.2))) 
			stop("Invalid specification of 'group' should be 4, 4.1, or 4.2!")
...
#-----------------------------------------------------------------------------------#	
		...
		if (!file.exists(paste("Output18\\", token, ".RData",sep=""))) {
			optCPF = list()
			if (hor == 12) par(mfrow=c(4,4), mar = c(2,1,.1,.1))
			else if (hor ==24) par(mfrow=c(4,3), mar = c(2,1,.1,.1))
			else if (hor ==6) par(mfrow=c(4,5), mar = c(2,1,.1,.1))
			...
			savePlot(file = paste("Plot18\\", token, "EF.emf", sep=""), type="emf") # Plots of Efficient Frontiers
			eval (parse(text = paste(token, '= optCPF')))
			fnm = paste("Output18/", token, ".Rdata", sep="")
			print(fnm)
			eval(parse(text = paste('save(', token,', file="',fnm,'" )',sep='')))
		} else load(paste("Output18\\", token ,".RData", sep=""))	
...
#-----------------------------------------------------------------------------------#
		...
		savePlot(file = paste("Plot18\\", token, "Wts.emf", sep=""), type="emf") 
		write.csv(wt, paste("Output18\\", token, "Wts.csv",sep=""))
		Sys.sleep(40); dev.off()
	}

#-----------------------------------------------------------------------------------#
#	Computing Cumulative Returns of one or more optimum PFs, added on 03-Sept-2014
#-----------------------------------------------------------------------------------#
	myPF_CumReturns = function(myPFstrList = c("optCPF3h1yr", "optCPF3h6mo"),bmRet = bmdebt, prefix="Conservative")
	{
		allRet = bmRet
		require(xts)
		...
		print(tail(allRet,10))
		win.graph(width=10, height=7, pointsize=11)
		par(mfrow=c(1,1), mar=c(4,4,2,2)) 
		chart.CumReturns(allRet, legend.loc="topleft", 
			main = "Comparing Cumulative Returns", 
				col= c(topo.colors(6)[1:length(myPFstrList)], c("yellow2", "gray", "yellow")),ylim=c(0.001, 0.09) )	
		savePlot(file = paste("Plot18\\", prefix, "CumRet.emf", sep=""), type="emf") # Plots of Cum Returns
		Sys.sleep(40);dev.off()
	}


#-----------------------------------------------------------------------------------#
#	Computing Quarterly Returns of one or more optimum PFs, added on 08-Sept-2014
#-----------------------------------------------------------------------------------#
	myPF_QrtReturns = function(myPFstrList = c("optCPF3h1yr","optCPF3h6mo" ,"optCPF5_1h6mo"),
			ttl = "Comparing Quarterly Returns for CPF3 (@H= 1yr, 6mo) & CPF5 (@H = 6mo)",bmRet = bmdebt, prefix="ConsPF3n5")
	{
		allRet = bmRet
		require(xts)
		for (pf in rev(myPFstrList)) {
		...
		for (ih in 1:length(tme6m$hstarts)) {
			...
			else  myQRet=rbind(myQRet,t(table.AnnualizedReturns(myRet, scale=261))[,1])
		}
	
		rownames(myQRet) = as.character(tme6m$Delhend)
		#print(head(myQRet))
		myCols=   c(topo.colors(6)[1:length(myPFstrList)], c("orange", "gray", "yellow"))
		win.graph(width=10, height=7, pointsize=11)
		par(mfrow=c(1,1), mar=c(4,4,2,2))
		barplot(t(myQRet), beside=T, legend.text=colnames(myQRet),las=2,cex.axis=.7, cex.names=1.5,
			main = ttl, col.main= "darkseagreen" ,col = myCols,
			ylim= c(min(myQRet,na.rm=T), max(myQRet,na.rm=T)*1.1),
			args.legend = list(x = "topleft") )
		savePlot(file = paste("Plot18\\", prefix, "QrtRet.emf", sep=""), type="emf") # Plots of Cum Returns
		Sys.sleep(40);dev.off()
	}

	
#-----------------------------------------------------------------------------------#
#	Computing Overall Performance of one or more optimum PFs, added on 08-Sept-2014
#-----------------------------------------------------------------------------------#
	myPF_Overall_Performance = function(myPFstrList = c("optCPF3h1yr","optCPF3h6mo" ,"optCPF5_1h6mo"),
			ttl = "Overall Performance of CPF3 (@H= 1yr, 6mo) & CPF5 (@H = 6mo)",
				risk = "SD",	bmRet = bmdebt, prefix="ConsPF3n5")
	{
		allRet = bmRet
		require(xts)
		for (pf in rev(myPFstrList)) {
...
#		ret2 = ret2["2016-01-01/2017-12-31"]
		...
		plot(annRskChk[,c(1:n)],annRetChk[,c(1:n)], pch=18, cex=2,col=myCols, ylim=c(0,max(annRetChk)),xlim=c(xmin,xmax),
			xlab=paste("Risk:",risk), ylab = "Annualized Return",	main= ttl,col.main= "black")
...
#		savePlot(file = "OverallPerfPF3&5xH1yr2yr6mo.emf", type="emf") 
		savePlot(file = paste("Plot18\\", prefix, "OverallPerf_",risk,".emf", sep=""), type="emf") # Plots of Cum Returns
		Sys.sleep(40);dev.off()
	}
