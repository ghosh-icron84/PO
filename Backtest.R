...
QQ = as.Date( startDate %m+% months(c(0:24)))
hends = as.Date((startDate  %m+% months(12)) %m+% months(c(0:24))) -1
Delhstr = hends +1
...
##finding optimum PF during 6mnth window##Training##Conservative PF##
tpf <- Tangency_PF_V4(RET=ret1, start="2016-01-01", end="2016-06-30", RF =0,n_Frontier=35,col.lgnd=lgnd,grp=4.2, dbg=0,robust=2)

wts = getWeights(tpf@portfolio); wts = wts[wts>0]
print(round(wts*100,0))

##testing during 3 mnth window##
ret2 = xts(ret1, order.by=as.Date(rownames(ret1)))
retNOW = ret2["2016-07-01/2016-09-30"]
retPF = retNOW[,match(names(wts),names(retNOW))]
head(retPF,2)
retPF1 = retPF %*% wts
retPF1 = xts(retPF1, order.by=as.Date(rownames(timeSeries(retPF))))
colnames(retPF1) = "optPF"

##Plotting the cumulative return of tPF during the Test Window##
head(retPF1,2)
chart.CumReturns(retPF1,legend.loc = "topleft",main = "Cumulative returns from July-Sept 2016",col = topo.colors(6)[1])
...
------------------------------------------------------------------------------------------------------
  ...
  pfBacktest_V2 = function(a.pf, RET=NULL, start=NULL, end=NULL, dbg=0)
  {
    ...
    sd <-  print(sd(dta1)*100) #  SD of daily return (risk) (pct) plotted in efficientFrontier
    ...
    CVaR = t(table.DownsideRisk(dta1))[,8]
...
----------------------------------------------------------------------------------------------
  ...
  pfBenchmark_V2 = function(BMRET=NULL, start=NULL, end=NULL, dbg=0)
  {
    ...
    CVaR = t(table.DownsideRisk(bmret1))[,8]
    data.frame(AnnRet,AnnRsk,maxDrawDown,CVaR)
  }
----------------------------------------------------------------------------------------------
...
for (ih in 1:length(hstarts)) {
  ...
  dta1 = dta %*% wts
  dta1 = xts(dta1, order.by=as.Date(rownames(timeSeries(dta))))
  bma1 = Select_RET_V2(RET=bmret, start=Delhstr[ih], end=Delhend[ih])[,c("S.P.BSE.100","S.P.BSE.MID.CAP")]
  {
...
ret2 = ret0[, - which(substr(lgnd$Asset.Type[match(colnames(ret0), lgnd$Legends)],1, 11) == "Debt+Equity")]
