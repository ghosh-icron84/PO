...
#		fastMveEst <- function(x, spec = NULL, ...) mveEstimator
	...
	if (grp==1)  { # only group constraints
		...
		paste("minW[1:",dim(my_asset)[2],"]=0.0",sep=""),
		paste("maxW[1:",dim(my_asset)[2],"]=0.4",sep=""),
		paste("minsumW[c(", paste(which(!is.na(match(grp.names, "Equity"))), collapse=","), ")]=0.40"),
		paste("maxsumW[c(", paste(which(!is.na(match(grp.names, "Equity"))), collapse=","), ")]=0.70"),
                paste("minsumW[c(", paste(which(!is.na(match(grp.names, "Debt"))), collapse=","), ")]=0.30"),
		paste("maxsumW[c(", paste(which(!is.na(match(grp.names, "Debt"))), collapse=","), ")]=0.60")
		)
...
# spent some time on this function on Sept 1 but final outcome is poor - so going back to v5 of myplot. 
	...
	#-------------------------------------------------------------------------------#
	# The function below is a wrapper to run various steps related to aggresive PF				
	...
	#-------------------------------------------------------------------------------#
	SMart_Wrapper_Q2OptPF = function(hor = 6, group = 92, method = "Basic", token = "optAPF3h1yr")
	{
		tme = BackTestingTimeFrames(horizon = hor) # computes the time windows for construction & testing 
		#if (!(group %in% c(4, 4.1, 4.2,92))) 
...
#-----------------------------------------------------------------------------------#
	myPF_CumReturns = function(myPFstrList = c("optAPF3_1h6mo", "optAPF3_1h1yr"), bmRet = bmeqt, prefix="Aggresive")
	{
		allRet = bmRet
		require(xts)
...
#-----------------------------------------------------------------------------------#
	myPF_QrtReturns = function(myPFstrList = c("optCPF3h1yr","optCPF3h6mo" ,"optCPF5_1h6mo"),
			ttl = "Comparing Quarterly Returns for CPF3 (@H= 1yr, 6mo) & CPF5 (@H = 6mo)",bmRet = bmeqt, prefix="Aggresive")
	{
		allRet = bmRet
		require(xts)
		for (pf in rev(myPFstrList)) {
