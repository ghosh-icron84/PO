...
attach(bmret)
#bmret <- xts(bmret,order.by=as.Date(rownames(bmret)))
 bmcomp = bmret[,c("CRISIL.Composite.Bond.Fund.Index","Nifty.50")]
bmcomp <- xts(bmcomp,order.by=as.Date(rownames(bmcomp)))
...
#-------------------------------------------------------------------------------#
setwd("C:/Users/debapriyag/Desktop/Portfolio Opt_2018/Code/Moderate_18/Codes")
source(file="UsefulFunctionsVQ_18.R")
...
#-------------------------------------------------------------------------------#
SMart_Wrapper_Q2OptPF(hor = 12, group = 93, method = "Basic", token = "optMPF3_1h1yr")
SMart_Wrapper_Q2OptPF(hor =  6, group = 93, method = "Basic", token = "optMPF3_1h6mo")
...
myPF_CumReturns(myPFstrList = c("optMPF3_1h1yr", "optMPF3_1h6mo"), bmRet = bmcomp, prefix="Moderate")
...
myPF_QrtReturns(myPFstrList = c("optMPF3_1h1yr", "optMPF3_1h6mo"),ttl = "Comparing Quarterly Returns for MPF3 (@H= 1yr, 6mo)",bmRet = bmcomp ,prefix="Moderate")
...
myPF_Overall_Performance(myPFstrList = c("optMPF3_1h1yr", "optMPF3_1h6mo"),ttl = "Overall Performance of MPF3 (@H= 1yr, 6mo)",risk = "SD",	bmRet = bmcomp, prefix="Moderate")

myPF_Overall_Performance(myPFstrList = c("optMPF3_1h1yr", "optMPF3_1h6mo"),ttl = "Overall Performance of MPF3 (@H= 1yr, 6mo)",
                         risk = "CVaR",	bmRet = bmcomp, prefix="Moderate")

myPF_Overall_Performance(myPFstrList = c("optMPF3_1h1yr", "optMPF3_1h6mo"),ttl = "Overall Performance of MPF3 (@H= 1yr, 6mo)",
                         risk = "MaxDrawDown",	bmRet = bmcomp, prefix="Moderate")
