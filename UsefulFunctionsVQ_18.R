...
#		fastMveEst <- function(x, spec = NULL, ...) mveEstimator
	...
	if (grp==1)  { # only group constraints
		...
		paste("minW[1:",dim(my_asset)[2],"]=0.0",sep=""),
		paste("maxW[1:",dim(my_asset)[2],"]=0.8",sep=""),
		paste("minsumW[c(", paste(which(!is.na(match(grp.names, "Debt"))), collapse=","), ")]=0.65"),
	...
	else if (grp==92)  { # addressing Q02 .... aggressive PF as requested (23.05.18)
		if (is.null(col.lgnd)) stop("must provide column legend info when grp !=0")
		grp.names = col.lgnd$Fund.Category[ match(colnames(my_asset), col.lgnd$Legends)]
		pfConstraints = c(# "LongOnly", ### warning - not working along with fund-level constraints!! 
		paste("minW[1:",dim(my_asset)[2],"]=0.0",sep=""),
		paste("maxW[1:",dim(my_asset)[2],"]=0.4",sep=""),
		paste("minsumW[c(", paste(which(!is.na(match(grp.names, "Equity"))), collapse=","), ")]=0.65"),
		paste("maxsumW[c(", paste(which(!is.na(match(grp.names, "Equity"))), collapse=","), ")]=0.80")
		)
	}	
         else if (grp==93)  { # addressing Q02 .... Moderate PF as requested (23.05.18)
		if (is.null(col.lgnd)) stop("must provide column legend info when grp !=0")
		grp.names = col.lgnd$Fund.Category[ match(colnames(my_asset), col.lgnd$Legends)]
		pfConstraints = c(# "LongOnly", ### warning - not working along with fund-level constraints!! 
		paste("minW[1:",dim(my_asset)[2],"]=0.0",sep=""),
		paste("maxW[1:",dim(my_asset)[2],"]=0.4",sep=""),
		paste("minsumW[c(", paste(which(!is.na(match(grp.names, "Equity"))), collapse=","), ")]=0.35"),
		paste("maxsumW[c(", paste(which(!is.na(match(grp.names, "Equity"))), collapse=","), ")]=0.50"),
                paste("minsumW[c(", paste(which(!is.na(match(grp.names, "Debt"))), collapse=","), ")]=0.5"),
		paste("maxsumW[c(", paste(which(!is.na(match(grp.names, "Debt"))), collapse=","), ")]=0.65")
		)
...
# spent some time on this function on Sept 1 but final outcome is poor - so going back to v5 of myplot. 
	...
	#-------------------------------------------------------------------------------#
	SMart_Wrapper_Q2OptPF = function(hor = 6, group = 93, method = "Basic", token = "optMPF3_1h6mo")
	{
...
#-----------------------------------------------------------------------------------#
	myPF_CumReturns = function(myPFstrList = c("optMPF3h_1h6mo", "optMPF3_1h1yr"), bmRet = bmcomp, prefix="Moderate")
	{
		allRet = bmRet
		require(xts)
...
#-----------------------------------------------------------------------------------#
	myPF_QrtReturns = function(myPFstrList = c("optMPF3_1h1yr", "optMPF3_1h6mo"),
			ttl = "Comparing Quarterly Returns for CPF3 (@H= 1yr, 6mo) & CPF5 (@H = 6mo)",bmRet = bmcomp, prefix="Moderate")
	{
		allRet = bmRet
		require(xts)
		for (pf in rev(myPFstrList)) {
...
#-----------------------------------------------------------------------------------#
	myPF_Overall_Performance = function(myPFstrList = c("optCPF3h1yr","optCPF3h6mo" ,"optCPF5_1h6mo"),
			ttl = "Overall Performance of CPF3 (@H= 1yr, 6mo) & CPF5 (@H = 6mo)",
				risk = "SD",	bmRet = bmeqt, prefix="Aggresive")
	{
		allRet = bmRet
		require(xts)
		for (pf in rev(myPFstrList)) {
