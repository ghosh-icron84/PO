...
#		fastMveEst <- function(x, spec = NULL, ...) mveEstimator
	...
	if (grp==1)  { # only group constraints
		...
		paste("minW[1:",dim(my_asset)[2],"]=0.0",sep=""),
		paste("maxW[1:",dim(my_asset)[2],"]=0.4",sep=""),
		paste("minsumW[c(", paste(which(!is.na(match(grp.names, "Equity"))), collapse=","), ")]=0.70"),
		paste("maxsumW[c(", paste(which(!is.na(match(grp.names, "Equity"))), collapse=","), ")]=0.90"),
                paste("minsumW[c(", paste(which(!is.na(match(grp.names, "Debt"))), collapse=","), ")]=0.10"),
		paste("maxsumW[c(", paste(which(!is.na(match(grp.names, "Debt"))), collapse=","), ")]=0.20")
		)
...
#-----------------------------------------------------------------------------------#
	myPF_CumReturns = function(myPFstrList = c("optCPF3h6mo", "optCPF3h1yr"), bmRet = bmeqt, prefix="Aggresive")
	{
		allRet = bmRet
		require(xts)
